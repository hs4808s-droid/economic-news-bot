...
- 실제 메일 발송은 Codex가 직접 하지 않는다. `4_email.md` 단계는 `logs/<날짜>/email_to_send.txt`(Subject 줄 + 빈 줄 + 본문)만 생성하고, `send_email.js`가 Gmail API로 그 파일을 읽어 발송한다 (Gmail 공식 MCP 커넥터는 create_draft만 지원해 자동 발송이 불가능하기 때문)
